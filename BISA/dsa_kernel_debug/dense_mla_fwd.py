...
from util import assert_tensors_similar
...
def ref_dense_mla_fwd_interface(Q, KV, offsets, sm_scale=None, is_casual=True):
    Q = Q.float()
    KV = KV.float()
    all_o = []
    for i in range(offsets.shape[0] - 1):
        ...
        # Causal mask for dense attention
        causal_mask = torch.tril(torch.ones(sq, sk, dtype=torch.bool, device="cuda"))
        mask = causal_mask.view(1, 1, 1, sq, sk)
...
def test_dense_mla_fwd(B=1,
    ...
    tl_out, tl_lse = dense_mla_fwd_interface(
        q, kv, offsets, block_I=block_I, num_stages=num_stages, threads=threads)

    if check_correctness:
        # otherwise may cause out of memory
        ref_out = ref_dense_mla_fwd_interface(q, kv, offsets)
        assert_tensors_similar(tl_out, ref_out, eps=1e-2, name="out")
        print("assert_tensors_similar passed")

    def fn():
        return dense_mla_fwd_interface(
            q, kv, offsets, block_I=block_I, num_stages=num_stages, threads=threads)
    ...
    # For dense attention, each query attends to all previous KVs (average S/2 for causal)
    avg_kv_len = S // 2
    print("fwd io bandwidth = ", (B * S * DQK * avg_kv_len * 2) / (ms * 1e-3) / 1e12)
    print("fwd tflops = ", (B * S * (DQK + DV) * avg_kv_len * 2 * H) / (ms * 1e-3) / 1e12)
